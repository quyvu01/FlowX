...
using FlowX.DynamicExpression.Visitors;

namespace FlowX.DynamicExpression;

/// <summary>
/// Class used to parse and compile a text expression into an Expression or a Delegate that can be invoked. Expression is written using a subset of C# syntax.
...
public class Interpreter
{
    ...
    /// </summary>
    public IEnumerable<Identifier> Identifiers => _settings.Identifiers
        .Select(p => p.Value)
        .ToList();
    ...
    /// <summary>
    /// Allow setting the default numeric type when no suffix is specified (Int by default, Double if real number)
    /// </summary>
    ...
    /// <summary>
    /// Allows enabling/disable assignment operators.
    /// For security when expressions are generated by the users is more safe to disable assignment operators.
    /// </summary>
    ...
    public Interpreter EnableReflection()
    {
        ...
        if (visitor != null) Visitors.Remove(visitor);
        return this;
    }
    ...
    /// Overloads can be added (i.e., multiple delegates can be registered with the same name).
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public Interpreter SetVariable<T>(string name, T value) => SetVariable(name, value, typeof(T));
    ...
    /// Add the specified expression as a known identifier.
    /// </summary>
    /// <param name="name"></param>
    /// <param name="expression"></param>
    /// <returns></returns>
    public Interpreter SetExpression(string name, Expression expression) =>
        SetIdentifier(new Identifier(name, expression));

    /// <summary>
    /// Allow the specified list of identifiers to be used in a parsed expression.
    /// Add the specified expressions as a known identifier.
    /// </summary>
    ...
    /// </summary>
    /// <param name="expressionText">Expression statement</param>
    /// <param name="expressionType">The expected return type. Use void or an object type if there isn't an expected return type.</param>
    /// <param name="parameters"></param>
    /// <returns></returns>
    /// <exception cref="ParseException"></exception>
    ...
    /// <param name="expressionText">Expression statement</param>
    /// <param name="parametersNames">Names of the parameters. If not specified, the parameter names defined inside the delegate are used.</param>
    /// <returns></returns>
    /// <exception cref="ParseException"></exception>
        ...
        params string[] parametersNames)
    {
        ...
        if (returnType == typeof(object)) returnType = typeof(void);
    ...
    public Lambda ParseAs<TDelegate>(string expressionText, params string[] parametersNames) =>
        ParseAs(typeof(TDelegate), expressionText, parametersNames);
    ...
    public object Eval(string expressionText, params Parameter[] parameters) =>
        Eval(expressionText, typeof(void), parameters);
    ...
    public object Eval(string expressionText, Type expressionType, params Parameter[] parameters) =>
        Parse(expressionText, expressionType, parameters).Invoke(parameters);
