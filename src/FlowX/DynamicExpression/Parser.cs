...
using System.Text;
using FlowX.DynamicExpression.Exceptions;
...
internal class Parser
{
    ...
    private const NumberStyles ParseLiteralDecimalNumberStyle =
        NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint;

    private const NumberStyles ParseLiteralDoubleNumberStyle =
    ...
    private ParameterWithPosition[] ParseLambdaParameterList()
    {
        ...
        if (!hasOpenParen && parameters.Length > 1)
            throw new ParseException("Multiple lambda parameters detected, but with no surrounding parenthesis",
                _parsePosition);
    ...
    private MethodData FindUnaryOperator(string operatorName, Expression expr)
    {
        ...
        if (applicableMethods.Count > 1)
            throw ParseException.Create(errorPos, "Ambiguous invocation of user defined operator '{0}' in type '{1}'",
                operatorName,
        ...
        while (true)
        {
            ...
            else if (_token.id == TokenId.Question && _parseChar is '.' or '[')
            {
                ...
                if (_token.id == TokenId.Dot)
                {
                    ...
                    var memberAccess =
                        GenerateNullableTypeConversion(ParseMemberAccess(GenerateGetNullableValue(expr)));
                ...
                else if (_token.id == TokenId.OpenBracket)
                {
                    ...
                    // the member access should be resolved on the underlying type
                    var elementAccess =
                        GenerateNullableTypeConversion(ParseElementAccess(GenerateGetNullableValue(expr)));
                    var nullExpr = ParserConstants.NullLiteralExpression;
                    CheckAndPromoteOperands(ParseSignatures.EqualitySignatures, ref expr, ref nullExpr);
                    expr = GenerateConditional(GenerateEqual(expr, nullExpr), ParserConstants.NullLiteralExpression,
    ...
    private Expression ParseParenExpression()
    {
        ...
        if (innerParenthesesExpression is ConstantExpression { Value: Type constType })
        {
            NextToken();
    ...
    private Expression ParseTypeof()
    {
        ...
        if (args.Length != 1)
            throw ParseException.Create(errorPos, "The 'typeof' keyword requires 1 argument");

        if (args[0] is not ConstantExpression { Value: Type } constExp)
            throw ParseException.Create(errorPos, "The 'typeof' keyword requires a type as an argument");
    ...
    private Expression ParseDefaultOperator()
    {
        ...
        ValidateToken(TokenId.OpenParen, "'.' or '(' expected");
        NextToken();
        ValidateToken(TokenId.Identifier);
    ...
    private Expression GenerateConditional(Expression test, Expression expr1, Expression expr2, int errorPos)
    {
        ...
        if (test.Type != typeof(bool))
            throw ParseException.Create(errorPos, "The first expression must be of type 'Boolean'");
        if (expr1.Type != expr2.Type)
        {
            var expr1As2 = expr2 != ParserConstants.NullLiteralExpression
                ? ExpressionUtils.PromoteExpression(expr1, expr2.Type)
                : null;
            var expr2As1 = expr1 != ParserConstants.NullLiteralExpression
                ? ExpressionUtils.PromoteExpression(expr2, expr1.Type)
                : null;
            ...
            else
            {
                ...
                if (expr1As2 != null)
                    throw ParseException.Create(errorPos, "Both of the types '{0}' and '{1}' convert to the other",
                        type1, type2);

                throw ParseException.Create(errorPos, "Neither of the types '{0}' and '{1}' converts to the other",
                    type1, type2);
            }
        }
    ...
    {
        NextToken();
        ValidateToken(TokenId.Identifier, "Type identifier expected");
    ...
    {
        return ParseArgumentList(TokenId.OpenCurlyBracket, "'{{' expected",
            TokenId.CloseCurlyBracket, "'}}' expected",
    ...
    private Expression ParseWithObjectInitializer(NewExpression newExpr, Type newType)
    {
        ...
        ValidateToken(TokenId.CloseCurlyBracket, "'}}' expected");
        NextToken();
    ...
    private void ParsePossibleMemberBinding(Type newType, List<MemberBinding> bindingList, List<Expression> actions,
        ParameterExpression instance, bool allowCollectionInit)
    {
        ...
        if (member == null)
        {
            throw ParseException.Create(pos, "No property or field '{0}' exists in type '{1}'", propertyOrFieldName,
                TypeUtils.GetTypeName(newType));
        }

        NextToken();

        ValidateToken(TokenId.Equal, "'=' expected");
        NextToken();
    ...
    private Expression ParseMethodGroupInvocation(MethodGroupExpression methodGroup, int errorPos)
    {
        ...
        if (applicableMethods.Count == 0)
        {
            ...
            applicableMethods = MethodResolution.FindBestMethod(candidates.Select(ov => ov.InvokeMethod), args);
        }
        ...
        if (applicableMethods.Count == 0)
        {
            if (args.Any(IsDynamicExpression))
            {
                ...
                var candidatesWithSameArgumentCount =
                    candidates.Where(o => o.Method.GetParameters().Length == args.Length).ToList();
            ...
            throw ParseException.Create(errorPos, "Argument list incompatible with delegate expression");
        }

        if (applicableMethods.Count > 1)
            throw ParseException.Create(errorPos, "Ambiguous invocation of delegate (multiple overloads found)");
    ...
    private List<Type> ParseTypeArgumentList()
    {
        ...
        ValidateToken(TokenId.GreaterThan, "'&gt;' expected");
        return args;
    }
    ...
    private Expression GenerateConversion(Expression expr, Type type, int errorPos)
    {
        ...
        {
            throw ParseException.Create(errorPos, "A value of type '{0}' cannot be converted to type '{1}'",
    ...
    private Expression GeneratePropertyOrFieldExpression(Type type, Expression instance, int errorPos,
        string propertyOrFieldName)
    {
        ...
        throw ParseException.Create(errorPos, "No property or field '{0}' exists in type '{1}'", propertyOrFieldName,
            TypeUtils.GetTypeName(type));
    }
    ...
    private Expression ParseExtensionMethodInvocation(Type type, Expression instance, int errorPos, string methodName,
        Expression[] args)
    {
        ...
        if (extensionMethods.Count > 1)
            throw ParseException.Create(errorPos, "Ambiguous invocation of method '{0}' in type '{1}'", methodName,
                TypeUtils.GetTypeName(type));
    ...
    private Expression ParseNormalMethodInvocation(Type type, Expression instance, int errorPos, string methodName,
        Expression[] args)
    {
        ...
        Expression[] args)
    {
        ...
        return Expression.Dynamic(new LateInvokeMethodCallSiteBinder(methodName, isStatic), typeof(object),
            argsDynamic);
    }
    ...
    private Expression ParseElementAccess(Expression expr)
    {
        ...
        if (expr.Type.IsArray)
        {
            if (expr.Type.GetArrayRank() != args.Length)
                throw ParseException.Create(errorPos, "Incorrect number of indexes");

            for (var i = 0; i < args.Length; i++)
            {
                args[i] = ExpressionUtils.PromoteExpression(args[i], typeof(int));
                if (args[i] == null)
                    throw ParseException.Create(errorPos, "Array index must be an integer expression");
            }
        ...
        if (applicableMethods.Count == 0)
        {
            throw ParseException.Create(errorPos, "No applicable indexer exists in type '{0}'",
                TypeUtils.GetTypeName(expr.Type));
        }

        if (applicableMethods.Count > 1)
        {
            throw ParseException.Create(errorPos, "Ambiguous invocation of indexer in type '{0}'",
                TypeUtils.GetTypeName(expr.Type));
        }
    ...
    private void CheckAndPromoteOperands(MethodBase[] binarySignatures, ref Expression left, ref Expression right)
    {
        ...
        else if (TypeUtils.TryGetNonNullableType(right.Type, out var nonNullableRightType) &&
                 nonNullableRightType == left.Type)
    ...
    private Expression GenerateBinaryDynamic(ExpressionType binaryType, Expression left, Expression right)
    {
        ...
        if (binaryType is ExpressionType.AndAlso or ExpressionType.OrElse)
        {
    ...
    private MethodData FindBinaryOperator(string operatorName, Expression left, Expression right)
    {
        ...
        ParseException CreateAmbiguousOperatorException() => ParseException.Create(errorPos,
            "Ambiguous invocation of user defined operator '{0}' in types '{1}' and '{2}'", operatorName,
            TypeUtils.GetTypeName(leftType),
            TypeUtils.GetTypeName(rightType));
        ...
        if (leftType != rightType)
        {
            ...
            if (userDefinedOperator != null && rightOperator != null &&
                !ReferenceEquals(userDefinedOperator.MethodBase, rightOperator.MethodBase))
                throw CreateAmbiguousOperatorException();
        ...
        switch (_parseChar)
        {
            ...
            case '.':
                ...
                if (char.IsDigit(_parseChar))
                {
                    ...
                    if (_parseChar is 'F' or 'f' or 'M' or 'm')
                        NextChar();
                    break;
                }
            ...
            case '"':
                ...
                if (_parsePosition == _expressionTextLength)
                    throw ParseException.Create(_parsePosition, "Unterminated string literal");
            ...
            default:
                ...
                if (char.IsDigit(_parseChar))
                {
                    ...
                    if (_parseChar == '0')
                    {
                        ...
                        else if (_parseChar is 'b' or 'B')
                        {
                            canBeRealLiteral = false;
                            do
                            {
                                NextChar();
                            } while (_parseChar is '0' or '1');
                    ...
                    if (canBeRealLiteral && _parseChar is 'E' or 'e')
                    {
                        t = TokenId.RealLiteral;
                        NextChar();
                        if (_parseChar is '+' or '-')
                    ...
                    if (canBeRealLiteral && _parseChar is 'D' or 'd' or 'F' or 'f' or 'M' or 'm')
                    {
                        t = TokenId.RealLiteral;
                        NextChar();
                    }
                    ...
                    if (_parseChar is 'U' or 'u')
                    {
                        t = TokenId.IntegerLiteral;
                        NextChar();
                        if (_parseChar is 'L' or 'l')
                            NextChar();
                    }
                    else if (_parseChar is 'L' or 'l')
                    {
                        t = TokenId.IntegerLiteral;
                        NextChar();
                        if (_parseChar is 'U' or 'u')
    ...
    {
        if (!char.IsDigit(_parseChar))
            throw ParseException.Create(_parsePosition, "Digit expected");
    }
