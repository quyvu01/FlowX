...
namespace FlowX.DynamicExpression
{
    ...
    internal class LateGetMemberCallSiteBinder(string propertyOrFieldName)
        : CallSiteBinder, IConvertibleToWritableBinder
    {
            ...
            LabelTarget returnLabel)
        {
            ...
            var binder = Binder.GetMember(
                ...
                parameters.Select(_ => CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null))
            );
    ...
    internal class LateInvokeMethodCallSiteBinder(string methodName, bool isStatic) : CallSiteBinder
    {
            ...
            LabelTarget returnLabel)
        {
            ...
            var argumentInfo = parameters.Select(_ => CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null))
                .ToArray();
            if (isStatic)
            {
    ...
    /// <summary>
    /// Binds to an item invocation of an instance as late as possible.  This allows the use of anonymous types on dynamic values.
    /// </summary>
