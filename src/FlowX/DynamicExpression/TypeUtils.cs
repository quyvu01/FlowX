...
{
    public static bool IsNullableType(Type type) => TryGetNonNullableType(type, out _);

    public static bool IsDynamicType(Type type) => typeof(IDynamicMetaObjectProvider).IsAssignableFrom(type);
    ...
    public static Type MakeNullable(Type type) => typeof(Nullable<>).MakeGenericType(type);
    ...
    public static bool IsNumericType(Type type) => GetNumericTypeKind(type) != 0;

    public static bool IsSignedIntegralType(Type type) => GetNumericTypeKind(type) == 2;

    public static bool IsUnsignedIntegralType(Type type) => GetNumericTypeKind(type) == 3;
    ...
    public static bool IsCompatibleWith(Type source, Type target)
    {
        ...
        if (target.IsGenericParameter) return true;
