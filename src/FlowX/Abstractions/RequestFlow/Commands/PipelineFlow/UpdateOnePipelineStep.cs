...
internal sealed class UpdateOnePipelineStep<TModel, TPrev> : IPipelineStepEntry
    where TModel : class
{
        ...
        CancellationToken ct)
    {
        ...
        if (model is null)
            throw NullError ?? (Exception)new NullReferenceException($"Entity {typeof(TModel).Name} not found");
        ...
        if (ModifyFunc is { } modifyFunc) await modifyFunc(model);
