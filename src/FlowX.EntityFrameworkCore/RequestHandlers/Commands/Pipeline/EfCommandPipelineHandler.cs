...
public abstract class EfCommandPipelineHandler<TCommand, TResult>
    : ICommandHandler<TCommand, TResult>
    where TCommand : class, ICommandResult<TResult>
{
    protected abstract IPipelineResultFlowBuilder<TResult> BuildPipeline(
        IStartCommandPipeline fromFlow, IRequestContext<TCommand> commandContext);
    ...
    public virtual async Task<TResult> HandleAsync(IRequestContext<TCommand> requestContext)
    {
        ...
        var builder = BuildPipeline(new CommandPipelineFlow(), requestContext);
...
public abstract class EfCommandPipelineHandler<TCommand> : ICommandHandler<TCommand>
    where TCommand : class, ICommandVoid
{
    protected abstract IPipelineFlowBuilder BuildPipeline(
        IStartCommandPipeline fromFlow, IRequestContext<TCommand> commandContext);
    ...
    public virtual async Task HandleAsync(IRequestContext<TCommand> requestContext)
    {
        ...
        if (builder.BeforeExecutionFunc is { } beforeFunc) await beforeFunc.Invoke();
