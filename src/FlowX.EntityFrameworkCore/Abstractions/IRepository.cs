...
public interface IRepository<T> where T : class
{
    ...
    Task<List<T>> CreateManyAsync(IEnumerable<T> items, CancellationToken token = default);
    ...
    Task<List<T>> RemoveManyAsync(IEnumerable<T> items, CancellationToken token = default);
}